<template>
  <div>
    ...
    <h1 style="color:#409eff ; width: 200px ; left: 845px;top: -50px; position: relative">搜索结果书目</h1>
...
export default {
  ...
  mounted(){
    ...
    let search = this.$cookies.get('search')
    const _this =this
    setTimeout(function () {
      if(type == 'all')
      {
        axios.get('http://localhost:8181/book/searchByAll/'+search).then(function (resp){
          _this.books = resp.data
        })
      }
      else if(type =='book_name')
      {
        axios.get('http://localhost:8181/book/searchByName/'+search).then(function (resp){
          _this.books = resp.data
        })
      }
      else if(type =='book_author')
      {
        axios.get('http://localhost:8181/book/searchByAuthor/'+search).then(function (resp){
          _this.books = resp.data
        })
      }
      else if(type =='publishing_house')
      {
        axios.get('http://localhost:8181/book/searchByPublish/'+search).then(function (resp){
          _this.books = resp.data
        })
      }
    },500);
