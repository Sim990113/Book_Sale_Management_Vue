<template>
  ...
  <div>
    <el-link :underline="false" href="/Login/Manager_Signin"><< 管理员通道</el-link>
      <el-form :model="LoginForm" :rules="users" ref="LoginForm" label-width="100px" class="LoginForm" >
        <el-row>
          <div style="background-color:#409eff;width:4px;height: 30px;left: 10%;top: 91px;position: relative;"></div>
          <label style="width: 20px;height: 20px;border-left: #409eff; position: relative;left: 12%;top: 60px;font-family: 'Adobe 黑体 Std R';font-weight: bold;font-size: 24px">用户登录</label>
        </el-row>
      <br><br><br><br>
        <el-row class="Logincontent">
          <el-form-item label="用户名:" prop="user_id" required >
            <el-input  v-model.number="LoginForm.user_id"  class="logrow" placeholder="用户名"></el-input>
          </el-form-item>
        </el-row>

        <el-row class="Logincontent">
            <el-form-item label="密码:" prop="user_password" required >
              <el-input  v-model="LoginForm.user_password"  type = 'password' class="logrow" placeholder="密码"></el-input>
            </el-form-item>
        </el-row>
...
export default {
  ...
  data(){
    return{
      ...
      users:{
        user_id:[
          {require:true, message:'请输入数字用户Id',trigger:'blur',type:'number'}
        ],
        user_password: [
  ...
  methods:{
    ...
    submitForm(formName) {
      const _this = this
        if(typeof this.LoginForm.user_id != 'number'){
          this.$notify({
            title:'提示',
            message: '请输入正确的用户Id',
            type:'warning'
        ...
        else if(this.LoginForm.user_id==''){
          this.$notify({
            title:'提示',
            message: '请填写用户Id',
            type:'warning'
          ...
          _this.$cookies.set('getid',0)
          axios.get('http://localhost:8181/user/findById/'+this.LoginForm.user_id).then(function (resp){
            _this.$cookies.set('getid',1)
            if(_this.LoginForm.user_password != resp.data.user_password)
              {
                console.log(resp)
                console.log(_this.LoginForm.user_password)
                _this.$notify({
                  title:'提示',
                  message: '请填写正确的密码',
                  type:'warning'
            ...
            else if(_this.LoginForm.user_password == resp.data.user_password && _this.identifyCode.toLowerCase() == _this.LoginForm.captcha_key.toLowerCase())
              {
                ...
                _this.$cookies.set('userid',_this.LoginForm.user_id)
                _this.$cookies.set('username',resp.data.user_name)
                _this.$router.push('/Home_Page/Page_Home')
                console.log(_this.$cookies.get('getid'))
              }
          })
          setTimeout(function () {
            if(_this.$cookies.get('getid')==0)
            {
              _this.$notify({
                title:'提示',
                message: '没有此用户，请注册',
