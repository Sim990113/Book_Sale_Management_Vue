<template>
  <div>
    <el-form :model="SignupForm" :rules="managers" ref="SignupForm" label-width="100px" class="SignupForm" >
      <el-row>
        <div style="background-color:#409eff;width:4px;height: 30px;left: 10%;top: 91px;position: relative;"></div>
        <label style="width: 20px;height: 20px;border-left: #409eff; position: relative;left: 12%;top: 60px;font-family: 'Adobe 黑体 Std R';font-weight: bold;font-size: 24px">新管理员注册</label>
      ...
      <el-row class="SignupContent">
        <el-form-item label="姓名:" prop="manager_name" required >
          <el-input  v-model="SignupForm.manager_name"  class="signuprow" placeholder="姓名"></el-input>
        </el-form-item>
      </el-row>

      <el-row class="SignupContent">
        <el-form-item label="设置密码:" prop="manager_password" required >
          <el-input  v-model="SignupForm.manager_password"  type = 'password' class="signuprow" placeholder="设置密码"></el-input>
        </el-form-item>
      </el-row>
      ...
      <el-row class="SignupContent">
        <el-form-item label="性别:" prop="manager_gender" required >
          <el-radio v-model="SignupForm.manager_gender" label="男">男</el-radio>
          <el-radio v-model="SignupForm.manager_gender" label="女">女</el-radio>
        </el-form-item>
      </el-row>
      ...
      <el-row class="SignupContent">
        <el-form-item label="认证号:" prop="key" required >
          <el-input  v-model="SignupForm.key" class="signuprow" placeholder="认证号"></el-input>
        </el-form-item>
      </el-row>
      <br><br><br>
...
export default {
  ...
  data(){
    return{
      ...
      managers:{
        ...
        key: [
          {require:true, message:'请输入正确的认证码',trigger:'blur'}
        ],
      },
    }
  ...
  methods: {
    ...
    submitForm(formName) {
      const _this = this
      if(this.SignupForm.key != 110)
      {
        this.$notify({
          title:'提示',
          message: '请输入正确的管理员认证号',
          type:'warning'
        })
      }
      else{
        //判断用户是否输入了中文
        if(escape(this.SignupForm.manager_name).indexOf( "%u" )<0){
          this.$notify({
            title:'提示',
            message: '请输入正确的姓名',
            type:'warning'
        ...
        else if(this.SignupForm.manager_name==''){
          this.$notify({
            title:'提示',
            message: '请填写姓名',
            type:'warning'
        ...
        else if(this.SignupForm.manager_password==''){
          this.$notify({
            title:'提示',
            message: '请设置密码',
            type:'warning'
        ...
        else if(this.SignupForm.manager_gender==''){
          this.$notify({
            title:'提示',
            message: '请选择性别',
            type:'warning'
        ...
        else if(this.SignupForm.manager_password != _this.SignupForm.sure_password){
          this.$notify({
            title:'提示',
            message: '请确认输入相同的密码',
            type:'warning'
        ...
        else if(_this.identifyCode.toLowerCase() != _this.SignupForm.captcha_key.toLowerCase())
        {
          ...
          axios.get('http://localhost:8181/manager/findByIdnum/'+this.SignupForm.idnum).then(function (resp){
            if(resp.data == '' ){
              axios.post('http://localhost:8181/manager/addmanager',_this.SignupForm).then(function (resp1){
              })
          ...
          setTimeout(function () {
            if(_this.sure == 1)
            {
              axios.get('http://localhost:8181/manager/findByIdnum/'+_this.SignupForm.idnum).then(function (resp){
                console.log(resp)
                console.log(resp.data.managerid)
                _this.$alert('注册成功，您的管理员ID是 '+resp.data.managerid+' ，请您牢记','提示',{
                  confirmButtonText:'确定',
                  type:'success',
                  callback:action => {
                    _this.$router.push('/Login/Manager_Signin')
                  }
                })
              })
            }
